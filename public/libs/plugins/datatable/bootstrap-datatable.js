  ...
  * ========================== */
  ...
  DataTable.prototype = {
    ...
    , details: function () {
        ...
        $('<div class="pull-left"><p>显示 ' + start + ' 至 ' + end + ' 条,总计: ' + o.totalRows + ' 条</p></div>')
    ...
    , row: function ( rowdata ) {
        ...
        // loop through the columns
        for(column in o.columns) {
          if(o.columns[column].title == '操作' || o.columns[column].title == 'action') break;
        ...
        if(o.rowAction){
        	$row.append(this.rowAction(rowdata));
        }
		...
		,rowAction:function(data){
			var field = this.options.rowAction.baseField;
			var updateUrl = this.options.rowAction.updateUrl.replace(/\{.\}/g,data[field]);
			var deleteUrl = this.options.rowAction.deleteUrl.replace(/\{.\}/g,data[field]);
     	return "<td><a href='"+updateUrl+"' class='btn btn-warning '><i class='icon-edit icon-white'></i> 修改</a>" +'&nbsp'+
     		"<a href='javascript:void(0)' deleteUrl='"+deleteUrl+"' class='btn btn-danger delete'><i class='icon-remove icon-white'></i> 删除</a>";
     	//"<input id='user-"+data[field]+"' type='button' class='btn btn-large delete' value='删除'></td>";
		}
